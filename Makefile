...
# NOTE: YOU WILL NEED TO SET THIS VARIABLE'S VALUE WHEN WORKING WITH HEIRARCHICAL DESIGNS
COMPONENT_FILES  := param_file.sv dataflow/* control_logic/* register.sv top8227.sv seven_seg_decoder.sv
...
$(FPGA_BUILD)/$(PROJ).json : $(ICE) $(addprefix $(SRC)/, $(COMPONENT_FILES) $(TOP_FILE)) $(PINMAP) $(SRC)/top.sv
	...
	@echo -e "----------------------------------------------------------------\n\n"
	@verilator --lint-only -Werror-WIDTH -Werror-SELRANGE -Werror-COMBDLY -Werror-LATCH -Werror-MULTIDRIVEN -Wno-UNOPTFLAT $(SRC)/top.sv $(addprefix $(SRC)/, $(COMPONENT_FILES) $(TOP_FILE))
	@mkdir -p $(FPGA_BUILD)
	@echo "----------------------------------------------------------------"
